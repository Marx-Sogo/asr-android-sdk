...
//    private boolean needOneshot = false;
    ...
    private void initOnlineAsrRecognizer(){
        mRecognizer = new OnlineRecognizer(this);
...
//                onEvent(SpeechConstants.Message.MSG_ASR_ONLINE_AUDIO_DATA,"", ShortByteUtil.shortArray2ByteArray(part),-1,part.length*2);
    ...
    private void handleEvent(String eventName, String param, byte[] data, int offset, int length){
        ...
        if(mListener!=null && mListener.get()!=null){
            mListener.get().onEvent(eventName,param,data,offset,length);
        }else{
...
//            handleError(SpeechConstants.ErrorDomain.ERR_ASR_ONLINE_PREPROCESS,code,msg,null);
            ...
            handleEvent(SpeechConstants.Message.MSG_ASR_ONLINE_SPEECH_END, null, null, 0,  0);
        }
        else if (code == VadTip.MSG_SPEECH_START.code){
            mSpeechFound = true;
            handleEvent(SpeechConstants.Message.MSG_ASR_ONLINE_SPEECH_START, null, null, 0,  0);
...
// AudioRecognizer的回调EventListener
    @Override
    public void onEvent(String eventName, String param, byte[] data, int offset, int length) {
        handleEvent(eventName,param,data,offset,length);
    }
